...
Requires a log file containing latitude, longitude and satellites_used to be passed as argument when executing the script
...
with open(logFile, 'r') as csvfile:
	...
	#Add lats and longs to their list
	for row in reader:
...
# Place map
gmap = gmplot.GoogleMapPlotter(meanLat, meanLong, 11)
...
for n in range(len(times)):
	if not (n%markerInterval) :
		gmap.marker(lats[n], longs[n], color = 'lightsalmon', title=times[n])
