...
def createGmap():
    ...
    return gmap
"""
FUNCTION : getListOfBlues
Gets all possible blues from accepted colours by gmplot and returns that list in a random order
IN
OUT
    :blueColours - list of strings
"""
def getListOfBlues():
    colourDict = html_color_codes
    blueNames = ['blue', 'aqua', 'cyan', 'marine', 'navy']
    blueColours = []

    for key in colourDict.keys():
        if any(x in key for x in blueNames):
            blueColours.append(key)

    random.shuffle(blueColours)
...
def plotPath(gmap):
    linkIndexes=[]
    n=0
    listOfBlues=getListOfBlues() #Get randomly arranged list of blue colours
